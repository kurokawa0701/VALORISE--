...
- **`@keyframes` は現在1つも使っていません。** 動きはすべて `transition` で処理しています（スクロール演出・アコーディオンの＋記号・ボタンのホバー）。演出を増やしたくなった場合は、まず `data-r` / `data-delay` の遅延の組み合わせで対応してください。
...
- **並び順**：VALORISE事業責任者 → CTO → プロジェクトエグゼクティブ → AIプロダクト開発責任者 → DX・マーケティング統括責任者。事業・技術・請負・AI開発・AI運用の順で、読み進めるほど「少人数で回せている理由」に寄っていく構成にしています。
...
- **`@keyframes` は2つまで。** 現在は0個です。過去にヒーローのエンブレムを1回だけ光らせる演出（`mark-shine`）を入れていましたが、ヒーローを写真に差し替えた際に削除しました。もし再び足す場合は、**ループさせないこと**（反復回数は `1`。`infinite` にすると一気に安っぽくなります）と、`prefers-reduced-motion: reduce` での無効化を必ずセットにしてください。
- **アニメーションの速度に関する教訓**：光沢の走査（シャイン）演出を5.2秒かけて動かしたところ、コードは正常に動作しているのに**静止して見える**という不具合が起きました。人の目は緩やかな輝度変化を動きとして知覚しません。この種の演出は**1.6秒以内**に収めてください。また `background-position` の `%` は「背景画像とボックスのサイズ差」に対する比率であり、`background-size: 260%` のときに帯が画面内に入るのは `81.25%`〜`18.75%` の区間だけです。この範囲を大きく外れる値まで振ると空振りします。
...
1. **画面上の `※要差し替え` バッジはすべて削除済みです。** 会社情報（設立・資本金・役員・社員数・所在地・電話番号・ISO認証・派遣事業許可）は運営会社の[会社情報ページ](https://neighborengineers.jp/company/)の記載を反映しました。ただし**次の項目は「情報が無いまま画面から消えている」状態**で、警告表示は出ません。公開前に必ず追記してください。
   ...
   - **news.html の VIDEO の説明文は、各動画の目次から要約したドラフトです。** 動画そのものは視聴していないため、内容と食い違っていないか確認してください。日付・タイトル・尺は実データです。
...
3. **お問い合わせフォームの送信先** — Googleスプレッドシートへ記録する仕組みを実装済みです。`contact.html` の `<form data-endpoint="">` に Apps Script のウェブアプリURLを貼れば有効になります。手順は本ファイル末尾の「お問い合わせフォームの接続」を参照してください。**URLが空のあいだは送信されず、画面に未設定である旨が表示されます。**
...
**TOPページのヒーローは写真です**（`assets/hero-night.jpg`／高層階から見下ろした東京の夜景。右奥に東京タワー）。

この写真を選ぶ／差し替えるときの条件は2つあります。

1. **左45%が暗く平坦であること。** 見出しがそこに乗ります。
2. **下25%が暗いこと。** リード文とボタンが乗ります。

**見出しを3行（MAKE / ENGINEERS / VALUED）にしているのは、この写真に合わせた判断です。** 1行にまとめた「MAKE ENGINEERS」はフォント幅が 7.46em あり、`font-size: 7.4vw` では画面幅にかかわらず必ず横55%まで伸びて、窓の外の明かりに重なります（実測 1.6:1）。3行に割ると最も長い「ENGINEERS」でも39%に収まります。**1行に戻す場合は写真も差し替えが必要です。**
...
白文字とのコントラストは、文字が載る領域を計算して確認済みです（基準は本文4.5:1）。

| 領域 | 最悪値 |
|---|---|
| ヒーロー 見出し（最悪値・全幅） | 19.1:1 |
| ヒーロー 下段リード | 21.0:1 |
| ヒーロー LEARN MORE ボタン | 7.96:1 |
| ABOUT 左半分 | 16.8:1 |

**左スクリム（`.scrim--hero-left`）は削除しました。** 現在の写真は左半分がほぼ黒で、スクリムを掛けても掛けなくてもコントラストの実測値が変わらなかったためです（最悪値はどちらも 6.62:1）。夜景をそのまま見せるほうが良いと判断しています。

**明るい左側を持つ写真に差し替える場合は、左スクリムの再追加が必要です。** 過去に窓越しの夜景を使ったときは、写真の34〜38%の位置にあった細い明かりの帯が「ENGINEERS」の行末と重なり、3.3:1 まで落ちました。写真を変えたら必ず再計測してください。

現在の弱点は右下の LEARN MORE ボタンです。ここだけ街の明かりに乗るため、`.scrim--bottom` の下端を `.62` から `.72` に強めて 7.96:1 を確保しています。緩めると 6.62:1 に落ちます。
...
送信内容をGoogleスプレッドシートに記録します。サーバーもデータベースも不要で、費用もかかりません。

### 手順

1. **スプレッドシートを新規作成**します。名前は「VALORISE お問い合わせ」など任意で構いません。
2. そのシートで **拡張機能 → Apps Script** を開きます。
3. エディタに元から入っている `function myFunction() {}` を**すべて消し**、`tools/gas-contact.gs` の中身を**まるごと貼り付け**ます。
4. ファイル上部の設定を確認します。
   - `NOTIFY_TO` … 社内で通知を受け取るメールアドレス。不要なら `''`（空）
   - `AUTO_REPLY` … 送信者への自動返信。`true` で有効、`false` で無効
   - `REPLY_TO` … 自動返信メールに返信されたときの宛先
   - `REPLY_FROM_NAME` … 自動返信の差出人の表示名
   - `SIGNATURE` … 自動返信の末尾に付く署名。1要素が1行です。担当者が変わったらここだけ書き換えます
5. 保存し、右上の **デプロイ → 新しいデプロイ** を選びます。
6. 歯車マークから **種類を「ウェブアプリ」** に設定し、次の2つを必ずこのとおりにします。
   - **次のユーザーとして実行**：自分
   - **アクセスできるユーザー：全員** ← ここが「自分のみ」だとサイトから送信できません
7. デプロイを実行し、表示された **ウェブアプリURL**（`https://script.google.com/macros/s/.../exec`）をコピーします。
8. `contact.html` の `<form ... data-endpoint="">` の `""` の中にそのURLを貼ります。

**2026年9月2日時点で、上記の設定は完了しています。** URLは `contact.html` に設定済みで、別ドメインからのPOSTが通ること（CORSが問題ないこと）も確認済みです。スクリプトを作り直してURLが変わった場合のみ、8を再度行ってください。

これで完了です。初回のデプロイ時にGoogleの承認画面が出ます。「このアプリは確認されていません」と警告が出ますが、自分で作ったスクリプトなので「詳細」→「（プロジェクト名）に移動」から承認して進めてください。

### 動作確認

1. コピーしたURLをブラウザで開くと `{"ok":true,"message":"..."}` と表示されます。ここでエラーが出る場合はデプロイ設定（手順6）を見直してください。
2. 実際に `contact.html` から送信し、シートに行が増えることを確認します。
3. Apps Scriptエディタで `testAppend` 関数を実行すると、ダミーの1件を記録できます。

### 送信されるメール

1件の送信につき、次の2通が自動で送られます。

| 宛先 | 件名 | 内容 |
|---|---|---|
| 社内（`NOTIFY_TO`） | `[VALORISE] お問い合わせ：（お名前）` | 全項目とスプレッドシートへのリンク |
| 送信者（`AUTO_REPLY`） | `【VALORISE】お問い合わせを受け付けました` | 受付のお礼、3営業日以内に連絡する旨、入力内容の控え |

自動返信には入力内容の控えを載せています。送信者が「何を送ったか」を後から確認できるようにするためです。

**メール送信数には1日あたりの上限があります。** Google Workspace アカウントで1日1,500通、無料のGmailアカウントで100通です。1件の問い合わせで2通消費するため、Workspaceなら1日750件まで対応できます。通常の運用で超えることはまずありませんが、Apps Scriptエディタで `checkMailQuota` 関数を実行すると残数を確認できます。

### 記録される項目

受信日時／お問い合わせ種別／お名前／会社名・組織名／メールアドレス／電話番号／ご相談内容／送信元ページ／対応状況

「対応状況」列は自動で `未対応` が入ります。**運用側で書き換える前提の列**です。ここを「対応中」「完了」などに更新して進捗管理に使ってください。

### 注意点

- **スクリプトを修正したら、必ず「デプロイ → デプロイを管理 → 編集（鉛筆）→ バージョン：新バージョン」で再デプロイしてください。** 保存しただけでは反映されません。ここは非常に間違えやすい箇所です。
- `script.js` の送信処理で `Content-Type` を `text/plain` にしているのは意図的です。`application/json` にするとブラウザがプリフライト（OPTIONS）を送り、Apps Script が応答できずCORSエラーになります。**変更しないでください。**
- `contact.html` の `.hp` ブロック（`f-website`）はスパム対策の隠し欄です。人には見えません。**削除しないでください。**
- ウェブアプリURLは公開情報です（HTMLに書くため誰でも見られます）。悪用されてもスプレッドシートに行が増えるだけで、シートの中身が読まれることはありません。大量送信が起きた場合は、デプロイを削除して作り直せば止まります。

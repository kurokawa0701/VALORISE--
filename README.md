...
| ファイル | 使用箇所 | 寸法 | 容量 |
|---|---|---|---|
| `assets/hero-night.jpg` | index ヒーロー背景（高層階から見下ろす夜景） | 1672×941 | 212KB |
...
**TOPページのヒーローは写真です**（`assets/hero-night.jpg`／高層階の窓から見下ろした夜景）。
...
| 領域 | 最悪値 |
|---|---|
| ヒーロー 見出し（最悪値・全幅） | 12.9:1 |
| ヒーロー 下段リード | 20.9:1 |
| ヒーロー LEARN MORE ボタン | 12.9:1 |
| ABOUT 左半分 | 16.8:1 |

`.scrim--hero-left` は単純なグラデーションではなく、42%まで `.86` で一定に保つ**平坦部つき**にしています。写真の34〜38%の位置に窓枠の隙間から差す細い明かりの帯があり、平坦部を外すと「ENGINEERS」の行末がそこに重なって **3.3:1** まで落ちます。写真かスクリムを変えたら必ず再計測してください。
